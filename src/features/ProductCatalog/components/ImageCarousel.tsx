...
const ImageCarousel = ({ productId }: Props) => {
    ...
    const images = product?.images && product.images.length > 0 ? product.images : [DEFAULT_IMAGE_URL];
    return (
        <div id={`carousel-${productId}`} className="relative w-full max-w-[480px] h-[480px] border border-[#E9E9E9] rounded-[8px] overflow-hidden flex items-center justify-center">
            ...
            {product?.images && (
                <img src={images[currentIndex]}   alt={`Product image ${currentIndex + 1}`} className="w-full h-full object-cover" 
                onError={(e) => {
